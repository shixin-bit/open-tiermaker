...
- [x] 3.2 实现 LocalStrategy（邮箱密码注册登录）：`AuthController` 暴露 `POST /auth/register` 和 `POST /auth/login`，密码用 bcrypt 哈希。`POST /auth/logout` 将 refresh token 加入 Redis 黑名单。`POST /auth/refresh` 实现 refresh token 轮换（换新 token，旧 token 标记已用防重放）。`GET /auth/session` 根据 access token 返回当前用户信息。验证：用 curl/Postman 走通注册→登录→刷新→登出完整链路
- [x] 3.3 实现 GitHub OAuth：`GET /auth/github` 重定向、`GET /auth/github/callback` 由 Passport GitHubStrategy 处理，成功后对 `Account` 表 upsert 关联，签发 refresh token 后 302 跳转到 `${FRONTEND_URL}/auth/callback?refresh=xxx`。验证：配置好 GitHub OAuth App 后能走通授权流程
- [x] 3.4 实现 Google OAuth：与 GitHub OAuth 流程对称，Passport GoogleStrategy，回调 302 跳转到前端。验证：配置好 Google OAuth Client 后能走通授权流程
- [x] 3.5 配置 CORS 中间件，允许 Cloudflare Workers 前端域名、允许携带 `Authorization` header、不允许 credentials（JWT 走 header 不走 Cookie）。验证：跨域 fetch 带 Authorization header 能成功请求
...
- [x] 4.3 实现图片上传：`POST /boards/:id/images` 接收 multipart/form-data，校验 MIME type（PNG/JPG/JPEG/GIF/WEBP）和大小（登录模式下 10MB 上限），存入 `BoardItem.imageData`（bytea）。`DELETE /boards/:id/images/:imgId` 移除图片。验证：上传图片后 GET board content 能取回二进制数据（前端用 blob URL 渲染）
...
- [x] 6.2 创建 API client 层：`packages/web/src/lib/api/client.ts` 封装 fetch，统一处理 baseURL（Vite dev 时 proxy 到后端，生产时指向 Cloudflare Workers）、自动附 `Authorization: Bearer` header、401 自动调 refresh token 重试一次。验证：单元测试覆盖 401 刷新重试逻辑
- [x] 6.3 创建 `useAuth()` Hook：从 `GET /api/auth/session` 获取当前用户信息，暴露 `user`、`isAuthenticated`、`loading`、`login(email, password)`、`register(email, password, username)`、`logout()`。access token 和 refresh token 存 localStorage（key: `otm:access` / `otm:refresh`）。验证：登录后刷新页面能自动恢复登录状态
...
- [x] 7.3 创建 `OAuthCallbackPage`（路由 `/auth/callback`）：从 URL query 获取 `refresh` 参数存 localStorage，然后调 `/api/auth/refresh` 换 access token，跳转首页。验证：GitHub/Google 授权后能正确回到前端并保持登录
- [x] 7.4 登出功能：header 区域新增"登出"按钮（仅已登录时显示），调 `POST /api/auth/logout` 然后清除 localStorage 中的 token，跳转首页。验证：登出后 `/api/auth/session` 返回 user: null
...
- [x] 9.2 重构图片渲染：登录模式下，图片通过 `GET /api/boards/:id/images/:imgId`（blob）转 URL 渲染；游客模式继续用 base64 直接渲染。验证：已上传图片刷新页面后仍能显示
...
- [x] 10.4 登录后数据迁移：`login()` 成功后立即执行 `migrateLocalToCloud()`——遍历 localStorage 中所有排行榜 → 逐个 `POST /api/boards` + `PUT /boards/:id/content` → 清空 localStorage 排行榜数据。验证：游客模式做了几个排行榜后登录，登录后云端能看到这些排行榜

## 11. 前端分享功能

- [x] 11.1 排行榜设置面板：编辑页侧边栏新增"分享"区域，包含可见性下拉（private / unlisted / public）、可选密码输入框、当前分享链接 + "重置链接"按钮。验证：切换可见性后端数据库正确更新
- [x] 11.2 未登录用户点击分享：点击分享区域任何操作触发登录 Modal（嵌入登录表单的对话框），登录成功后自动继续原来的分享操作。验证：游客点击分享→弹登录框→登录成功后分享功能可用
